...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), Geist for all
  display + Geist body + Geist Mono for the index/code voice on a warm-navy code surface,
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "Geist", cqw: 1.5, weight: 400, lineHeight: 1.5 }
  lead:    { fontFamily: "Geist", cqw: 2.08, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "Geist", cqw: 2.3, weight: 500, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "Geist", cqw: 1.46, weight: 500, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "Geist", cqw: 1.35, weight: 500, tracking: "0.18em", upper: true }
  kicker:  { fontFamily: "Geist Mono", px: 28, cqw: 1.46, weight: 500, tracking: "0.16em", upper: true }
  mono-label:{ fontFamily: "Geist Mono", px: 26, cqw: 1.35, weight: 500, tracking: "0.02em" }
  code:    { fontFamily: "Geist Mono", cqw: 1.67, weight: 400, lineHeight: 1.6 }
  # — display ramp (Geist 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "Geist", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "Geist", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "Geist", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "Geist", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "Geist", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "Geist", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
  number-unit:{ fontFamily: "Geist Mono", cqw: 2.08, weight: 500, lineHeight: 1.0 }
...
components:
  ...
  kicker-spike:
    ...
    description: "The eyebrow — Geist Mono uppercase, indexical (2–5 words), prefixed with the coral ✱. Never plain text, never a sentence."
  coral-callout:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Geist figure paired with a Geist Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Geist italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. Geist italic, small Geist uppercase cite beneath."
  section-rule:
  ...
  code-surface:
    ...
    textColor: "{colors.cream} (Geist Mono); syntax in coral (keywords) / teal #5DB8A6 (strings) / amber #E8A55A (numbers)"
...
Three editorial voices, each in its own face: **Geist** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **Geist** carries body, leads, card titles, buttons, and
UI chrome. **Geist Mono** carries the indexical layer — kickers, technical labels, the code
...
- **Geist** (sentence case, negative-tracked) for all display; **Geist** body/chrome; **Geist Mono** index + code.
...
- **Squint** — one Geist display moment dominates at 3–6× its neighbor.
...
- **Type** — Geist sentence-case display (negative-tracked); Geist 400 body; Geist Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (Geist `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Geist `headline` 4.6cqw
...
- **Geist display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Geist body** sentence case weight 400. **Geist Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — Geist figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Geist italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized Geist · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Geist `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** Geist 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single Geist line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Geist `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
- Set all display in **Geist, sentence case**, negative-tracked; **Geist 400** body; **Geist Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a Geist figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Geist display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one Geist display moment dominates.
...
- **Type** — Geist sentence-case display (negative-tracked); Geist 400 body; Geist Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
